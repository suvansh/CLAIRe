...
async function respondNow(input: string, history: IMessage[], datetime: Moment, chroma: Chroma, memory: ClaireMemory, chat: ChatOpenAI, user: string, res: NextApiResponse): Promise<string> {
    ...
    const response = await chain.predict({ input: input, user: user, datetime: datetimeString, history: history.map(messageToLangChainMessage) });
...
async function respondLater(input: string, history: IMessage[], responseNow: string, datetime: Moment, chroma: Chroma, memory: ClaireMemory, model: string, user: string) {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    const { chroma, memory } = await getMemory(user);
